...
해석이 어려움 : sensitivity anlysis를 통해 해석하려는 노력

- 최근에는 XAI (explainable AI)로 SHAP나 LIME 등 해석하기 위한 노력
...
- black box : not easily interpreted
...
sensitive inputs can be treated as important predictors
...
로지스틱은 하나의 관측값이 변해도 큰 차이가 없으나, 뉴럴네트워크나 의사결정나무는 크게 변화한다.

    - instability : hyperplane의 변동성
    
의사결정 나무, 머신러닝 : bias는 작으나 variance가 큼

앙상블 : 위의 모형을 이용하여 분산을 줄여 bias와 variance가 작음
