...
nn1<-neuralnet(RESPOND~AGE+BUY18+CLIMATE+FICO+INCOME+MARRIED+OWNHOME+GENDER, 
               data=train, hidden=3, stepmax = 1e+05, threshold = 0.01, 
...
roccurve3 <- roc(test$RESPOND ~ as.vector(pred3$net.result))
plot(roccurve1, col="red", print.auc=TRUE, print.auc.adj=c(1,7), auc.polygon=TRUE)
plot(roccurve3, col="purple", add=TRUE,  print.auc=TRUE, print.auc.adj=c(3,0))
legend("bottomright", 
       legend=c("one layer","one layer 2","two layers","two layers 2","logistic"), 
       col=c("red","blue","purple","black","darkgreen"), lwd=2)
```
