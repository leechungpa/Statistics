...
시그모이드 함수는 비선형이나 cut-off에 따른 분류경계선 자체는 선형이므로 선형모델임 (GLM : 초평면에서 선형이 나타나기 떄문)
...
3. 분할 후 각각의 노드별 분순도를 표본수에 따라 가중평균함

4. 분할 이후와 이전이 불순도가 감소했는지를 측정

5. greedy search를 통해 최적의 분할을 찾음 : 여러개의 변수 전부 무차별 대입



### CART의 faature selection 기준

다음의 parameter를 조정하여 최적의 feature selection을 찾음
...
missing value가 있을 경우 다른 추가적인 대안 분할 기준을 만들어, 이를 바탕으로 분할
...
- 좋은 결과를 위해 많이 대입해 시도해봐야 하며, 학습에 많은 시간이 걸림
...
- 범주형의 경우 class마다 0~1을 가지는 하나의 output node
...
2. NN는 input data에 매우 민감한 방법 : 좋은 데이터 타입 필요
...
classification on unweighted majority voting -> 분산을 감소시키는 효과
...
다중선형 회귀분석, 로지스틱 회귀분석과는 달리 개별 변수가 통계적으로 얼마나 유의한지에 대한 정보인 p-value 없음
...
이전과 다르게 밀도(densely populated area) 기반 군집 분석
