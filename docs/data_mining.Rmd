...
## hidden nodes는 2개로 구성되어 있음 : combination function + activation function 활성함수
...
bagging use unweighted majority voting : 집단지성 아이디어와 유사 -> 분산을 감소시키는 효과
...
- sample을 복원추출하여 이를 바탕으로 각각 의사결정 나무 시행

- 다수결의 방법을 통해 결과 취합

즉 bagging을 통해 분산을 줄임



각 학습 데이터셋은 전체 데이터 집합에서 복원추출을 통해 원래 데이터 수만큼의 크기를 갖도록 생성

이렇게 복원추출된 데이터셋을 boostrap이라 명명

각 의사결정나무는 서로 다른 학습 데이터셋을 사용하여 구축됨

최종 예측은 각 의사결정나무의 예측결과를 취합하여 생성


Bootstrap sample training set : $T^{(1)},\ ...\ , T^{(B)}$
Classifiers for each sample : $C_1(x,T^{(1)}),\ ...\ , C_B(x,T^{(B)})$
the number of times that classified as j : $N_j=\sum ^B_{b=1}I[ C_b(x,T^{(b)})=y_j]$
...
bagging을 할 때는 pruning 할 떄는 pruning 하지 않고 tree를 크게 만듦 : 분산이 커져 변동성이 증가하나 bias는 작아짐
...
비선형 모델의 변수중요도 : 각가의 tree마다의 변수중요도를 통합한 것 -> 이 결과를 NN에도 사용 가능함
...
Reduce the variance and the bias : bias가 더 크게 감소


오분류 관측치의 가중치를 높임 (전체 가중치의 합은 1)

결국 분류경계선 근처의 가중치가 커짐


기본아이디어
perform iteraitve search to locate the regions(examples) that are more difficult to preict -> bias를 줄임
through each itreation reward accurate predictions on those regions
...
Idea:
train a classifier f1 on data,
then train a f2 to correct the errors of f1,
then f3 to correct the errors of f2, etc
...
For m=1 to M
Fit a classifier $C_m(x)$ to T using weight
compute $err_m={\sum^N_{i=1}w_iI(y_i\not= C_m(X)i)) \over \sum^N_{i=1}w_i}$ : 오분류시 I=1
compute $\alpha_m=log((1-err_m)/err_m)$ : tree의 가중치 -> error가 적으면 $\alpha$상승
set $w_i=w_i\ \text{exp}[\alpha_mI(y_i\not= C_m(X_i))]$
$\sum w_i=1$이 되게 조정

Outpout $C_{AD}(X)=sign[\sum^M_{m=1}\alpha_mC_M(X)]$
...
$Y=h_1(X)+err1$에서 $err1=h_2(X)+err2$하는 형식으로 error 를 바탕으로 boosting을 계속하는 방법 : $F_m(X)=F_{m-1}(X)+w_mh_m(X)$
...
의사결정나무를 이용한 배깅의 특수한 형태를 취하는 앙상블 기법

랜덤포레스트는 앙상블의 다양성 확보하기 위해 특별한 방법을 사용 : 변수의 임의 추출

즉 random forest : 배깅의 boostrap + 변수 임의 추출
...
트리들이 **서로 독립이** 되도록 하고자 함

각 노드를 분할할 때, p개의 변수 중에서 탐색하지 않고, m개 (m<p)의 변수 중에서 탐색하여 분할함 (m=p이면 배깅과 동일)

m의 값이 작으면 각 나무모형들 간의 상관관계가 감소함 (m이 너무 적으면 정확도가 낮아짐)

보통 분류 data인 경우 $m=\sqrt{p}$. 회귀 data인 경우 $p\over 3$
...
군집화의 기준 : 동일한 군집에 속하면 여러 속성이 비슷하고, 다른군집에 속한 관측치는 그렇지 않음

얼마나 유사한지 비유사한지 측도 필요 : 유사성보다는 비유사성dissimilarity를 기준으로 하여 distance 사용
...
군집분석시 반드시 표준화 standardization 필요

군집분석은 자료 사이의 거리를 이용하여 수행되기 때문에, 각 자료의 단위가 결과에 큰 영향을 미친다. -> 표준화 방법 :  각 변수의 관찰값으로부터 그 변수의 평균을 빼고, 그 변수의 표준편차로 나누는 것
...
1. 먼저 표본수 그대로인 1개의 entity를 가지는 N 개의 군집으로 시작하여, NxN symmetric matrix of distances를 생성 : $D=\{d_{ik}\}$

2. distance matrix D의 원소중 가장 가까운 군집의 쌍 U와 V를 찾아 (UV)라는 하나의 군집으로 합침


3.  distance matrix d중 새롭게 변화 되는 부분인 distance between (UV) and other cluster W를 바꿈 : change $d_{(UV)W}$

4. 위의 과정을 N-1 반복하여 모든 관측치가 하나의 군집으로 바꿈
...
사전에 결정된 군집수 k에 기초하여 전체 데이터를 상대적으로 유사한 k개의 군집으로 구분한다.
...
군집별로 centroid 하여 새 군집의 초기값 설정 -> 재할당 반복
...
자료의 시각화를 위하여는 차원의 축소가 필수적이고, 이를 위하여 주성분 분석PCA방법이 널리 사용된다.



3. 대용량데이터에서 sampling한 데이터로 계층적 군집분석을 우선 수행하여, K의 값을 선택
...
K-means defines a prototype in terms of a centroid, usually the mean of a group of points.
...
-초기값에 영향을 많이 받음 : 초기 값이 다르면 다른 결과 가능

단점
- 초기 군집수 K를 결정하기 어려움 : 주관적 선택 필요
- 군집결과의 해석이 용이하지 않을 수 있음 (계층적 분석은 해석에 용이)



일반적으로 k-means는 군집수가 비슷하게 나옴 : 초기값을 잘못 정하면 잘못된 결과 가능
...
Within group sum of squares by cluster : 군집중심과 군집내 관찰값 간의 거리제곱의 합

급격히 감소하다 완만해지는 부분이 elbow point
...
K-medoid defines a prototype in terms of a medoid

medoid: the object whose average dissimilarity to all the objects in the cluster is minimal. i.e. it is the most centrally located point in the cluster 변수별 중앙값의 좌표(다변량)

more robust to noise and outliers because it minimizes a sum of pairwise dissimilarities instead of a sum of squared Euclidean distances.
...
Idea: group objects into one cluster if they are connected to one another by densely populated area : 이전에는 거리기반, 여기서는 밀도기반
...
A point q is dense point 기준점 if the # in the neighborhood is larger than Min Points


The neighborhood are put into the same cluster as q
